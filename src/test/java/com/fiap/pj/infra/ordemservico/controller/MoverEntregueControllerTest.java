...
class MoverEntregueControllerTest {

    @Mock
    private MoverEntregueUseCase moverEntregueUseCase;
    ...
    void deveMoverEmDiagnostico() throws Exception {
        mock.perform(post(
                TestUtils.buildURL(OrdemServicoController.PATH, UUID.randomUUID().toString(), "entregar"))
                .contentType(MediaType.APPLICATION_JSON_VALUE)
        ).andExpect(status().is2xxSuccessful());


        Mockito.verify(moverEntregueUseCase).handle(Mockito.any(UUID.class));
